...
### 3.9а. Вход на сайт через Telegram: `GET` и `POST /api/bot/auth-login`

Добавлено 20.09.2026 после разбора команды бота: эндпоинт был в коде,
но в ТЗ не описан. Тогда же исправлен и сам контракт — см. врезку ниже.

**Шаг 1 — посмотреть, не подтверждая.**

```
GET /api/bot/auth-login?nonce=<nonce>&telegramId=<id>
X-Bot-Api-Key: <ключ>
```
...
```json
{ "success": true, "data": {
  "maskedEmail": "i***v@mail.ru",
  "confirmCode": "4821",
  "request": { "ip": "…", "userAgent": "…", "createdAt": "…", "expiresAt": "…" }
}}
```

`404 NOT_LINKED` — у этого Telegram нет аккаунта на сайте.
`404 NONCE_INVALID` — ключа нет, он истёк или уже использован.

Ничего не меняет: вызывать можно повторно.

**Шаг 2 — подтвердить, после нажатия человеком «Подтвердить».**

```
POST /api/bot/auth-login   { "telegramId": "…", "nonce": "…" }
```

`200` — те же поля плюс `userId`. Те же две ошибки `404`. Подтвердить
можно один раз; повтор даёт `NONCE_INVALID`.

> **ПОЧЕМУ ШАГОВ ДВА.** До 20.09.2026 был только `POST`: он подтверждал
> вход и лишь ПОСЛЕ этого возвращал четыре цифры. Показать их человеку
> до подтверждения было нечем — а весь смысл цифр в том, чтобы он
> сверил их заранее. Иначе злоумышленник открывает вход у себя, шлёт
> жертве ссылку `t.me/…?start=tglogin_<свой nonce>`, и одно нажатие
> «Старт» впускает его в чужой аккаунт.
>
> **Бот обязан:** вызвать `GET`, показать код и устройство, дождаться
> нажатия и только тогда вызвать `POST`. Вызывать `POST` сразу на
> `/start` нельзя.

**Таймауты:** оба — 10 с. `GET` — до 2 повторов, `POST` — без
повторов (повтор после успеха вернёт `NONCE_INVALID`, это не ошибка:
...
| `email/confirm`, `POST link`, `relink` | 30 с | на таймаут/5xx/503 — до 3 раз, паузы ≈2–3, 5, 10 с, тем же телом |
...
**Паузы между повторами — ориентир, а не буква** (уточнено 20.09.2026).
Если общий `retry_async` бота даёт 2,5 / 5 / 10 вместо 2 / 5 / 10 —
так и оставить. Вторую реализацию ретраев ради точного совпадения с
таблицей заводить нельзя: один слой повторов на вызов, и он общий.

**ГДЕ ЛЕЖАТ ПОЛЯ ОШИБКИ.** `code`, `error`, а также `retryAfterSeconds`
(3.1) и `attemptsLeft` (3.2) — на ВЕРХНЕМ уровне ответа, не внутри
`data`. Сверено с кодом сайта 20.09.2026 (`src/app/api/bot/link-http.ts`,
`linkErrorResponse`):

```json
{ "success": false, "error": "Неверный код. Осталось попыток: 3",
  "code": "CODE_INVALID", "attemptsLeft": 3 }
```

Разбирать только `data` нельзя: человек с тремя оставшимися попытками
увидит «Осталось попыток: 0». Заодно сайт ставит заголовок
`Retry-After` при `429` и при `503` — им тоже можно пользоваться.
...
**Работа бота:** после подтверждения почты кодом записать эту почту в
свои сущности в панели и поставить маркер `atlas-email-verified`.

**КАКУЮ СУЩНОСТЬ ШТАМПОВАТЬ** (уточнено 20.09.2026 по разбору команды
бота — в ТЗ этого не было, и вопрос законный):

- **обход `{telegram_id}` — всегда.** Именно ради него штамп и нужен:
  по адресу почты сайт находит ключ 2, иначе он найдёт только премиум;
- **премиум — только пока он не сайтовый.** У связанного человека
  премиум-сущность либо принадлежит сайту (на ней стоит `atlas-site:`),
  либо отключена. Ни ту, ни другую бот не трогает.

**ПОЧЕМУ ЭТОТ ПУТЬ — ЗАПАСНОЙ, А НЕ ГЛАВНЫЙ.** Подтверждение кода в
боте (`email/confirm`) САМО создаёт аккаунт на сайте и связывает его —
`linkTelegramAccount`, поле `created` в ответе. То есть человек, дошедший
до конца связки в боте, на сайте уже есть, и усыновлять при входе нечего:
он просто входит по коду в свой аккаунт.

Усыновление по адресу остаётся на случай, когда в панели есть сущность с
подтверждённой почтой, а аккаунта на сайте нет: аккаунт удалили, связку
откатили, база была восстановлена из копии. Тогда человек входит по почте
и получает свой ключ обратно, а не пустой новый аккаунт.

**Отдельного «штампа почты до связки» в замысле не было.** Такой триггер
означал бы, что бот пишет адрес в панель, не создавая аккаунта на сайте,
и тогда бонус за связку, сложение дней и сама привязка переезжали бы на
момент первого входа на сайт — больше движущихся частей ради того же
результата. Делать не нужно.
...
| Что | Сейчас | Нужно |
|---|---|---|
| Ключ обхода при усыновлении | ~~берётся только премиум~~ **сделано 20.09.2026** (коммит `166b5ee`): усыновление подхватывает и `{telegram_id}` — он становится ключом 2 на сайте | — |
| Сложение дней | ~~остаётся больший срок~~ **сделано 20.09.2026** (коммит `166b5ee`): складываются остатки (принцип 4) | — |
...
3. Бот разбирает `^tglogin_[A-Za-z0-9_-]{16,64}$` и вызывает
   **`GET /api/bot/auth-login`** (раздел 3.9а) — смотрит, не подтверждая.
   Сайт возвращает те же четыре цифры, маскированный адрес, IP и
   браузер, с которых просят вход.
4. Бот показывает их человеку: «Подтвердить вход на сайте? Код 4821,
   устройство …». [✅ Подтвердить] [Отмена].
5. Нажали «Подтвердить» → **`POST /api/bot/auth-login`**. Страница
   сайта это видит и впускает человека. «Отмена» — не вызывать ничего,
   ключ протухнет сам через пять минут.
...
подтверждать нельзя. Именно поэтому шагов два (3.9а): показать можно
только то, что получено ДО подтверждения.
...
1. `days`:
   - есть `context.premium_minutes` → `premium_minutes / 1440`;
   - иначе `context.premium_days`.

   **Минуты имеют приоритет, а не служат запасным вариантом** (правка
   20.09.2026 по разбору команды бота). Прежняя формулировка — «days =
   premium_days, если > 0, иначе minutes / 1440» — была недостижима:
   `tariffs.for_grant_duration` округляет выдачу меньше суток ВВЕРХ до
   одного дня, поэтому `premium_days` у минутной выдачи никогда не ноль
   и ветка с минутами не срабатывала ни разу. Десятиминутная админская
   выдача ушла бы на сайт как целые сутки — в 144 раза больше.

   Бот обязан класть `premium_minutes` в `context` задачи для минутных
   админских выдач (этап A это уже делает).

   Дробные `days` сайт принимает (см. 3.7): десять минут — это
   `0.00694`. Ограничение `0 < days ≤ 400` сохраняется.
...
> **Решение владельца 20.09.2026: передачу кешбэка оставляем как есть.**
> Обсуждалось, не отказаться ли от синхронизации кешбэка и рефералов —
> «это сложно». Отказ разобрали и отклонили по факту из кода сайта:
> **баланс на сайте потратить нельзя.** В создании платежа он не
> участвует вовсе — сайт только начисляет кешбэк и показывает число в
> кабинете, а настоящими деньгами оно становится, когда их забирает бот.
>
> Значит «не синхронизировать» означало бы либо начислять кешбэк, который
> никто никогда не потратит (на главной висит обещание 10/25/45%), либо
> снимать обещание кешбэка с сайта целиком. Первое — невыполненное
> обещание, второе — потеря того, ради чего кешбэк заводили.
>
> К тому же сложность уже оплачена: обмен написан и закрыт тестами на
> этапе C, и он в любом случае недостижим, пока выключен
> `SITE_LINK_ENABLED`.
...
- `app/services/site_link/client.py` — HTTP-клиент: `status`, `email_start`,
  ...
  `auth_login_peek` (GET 3.9а), `auth_login_confirm` (POST 3.9а).
...
- `app/handlers/user/site_link.py` — FSM 4.1, подтверждение 4.2, экраны 4.3–4.6,
  а также подтверждение входа на сайт (4а.2): разбор `tglogin_…` в `/start`,
  показ кода и устройства по `GET /api/bot/auth-login`, кнопки
  [Подтвердить] / [Отмена], вызов `POST` только по нажатию.
...
- `SITE_LINK_ENABLED` — выключатель, по умолчанию `false`. Выключен → кнопки
  нет, `/start link_` отвечает «Скоро», воркер не стартует.

  **ЧТО ФЛАГ НЕ ВЫКЛЮЧАЕТ — и это главное про него.** Развилка 5.2 и все
  защиты раздела 6 работают ВСЕГДА, независимо от флага. Флаг гасит поток
  связки — то, чем человек пользуется; он не гасит защиты — то, чем
  защищён уже связанный.

  Если свести флаг к одному гейту на всю подсистему, то после его
  выключения бот у уже связанного человека снова начнёт писать в общий
  ключ напрямую: сроки на сайте и в панели разъедутся, оплаченные дни
  пропадут. Выключатель для того и нужен, чтобы им можно было
  воспользоваться в любой момент, — значит выключение обязано быть
  безопасным.

  Правило уточнено 20.09.2026 по разбору команды бота: в коде это по
  строению не видно, и следующий агент может «упростить» флаг именно так.
  Поэтому то же самое должно быть написано в `CLAUDE.md` бота.
...
- [ ] Вход через Telegram (4а.2): `/start tglogin_<nonce>` → `GET auth-login` показывает код и устройство, вход ещё НЕ подтверждён (повторный `GET` отвечает тем же); «Подтвердить» → `POST` → сайт впускает.
- [ ] Вход через Telegram, отмена: `POST` не вызывается, через 5 минут ключ протух, сайт показывает «Время вышло».
- [ ] Вход через Telegram у непривязанного Telegram → `404 NOT_LINKED`, боту показать «Сначала привяжите почту».
- [ ] Повторный `POST auth-login` тем же ключом → `404 NONCE_INVALID` (не ошибка: вход уже подтверждён).
...
## 16. Отвязка с выбором стороны и самолечение ключа обхода

Внесено 20.09.2026 по разбору владельца на живом человеке. Два разных
дела, связанных одной причиной: после связки часть имущества человека
осталась не там, где он её ищет.

### 16.1. Что случилось у живого человека

- в боте были И подписка, И гигабайты обхода;
- связка прошла, **подписка синхронизировалась**;
- **гигабайты обхода — нет**: на сайте ключа 2 не видно, в боте он есть;
- повторная привязка почты положение не исправила.

### 16.2. Почему обход мог не приехать (сторона сайта)

Разбор кода `src/lib/telegram-link.ts`:

1. Обход ищется ОДИН РАЗ, в момент связки: `findBotBypass(telegramId)`
   ищет сущность с username `{telegram_id}`.
2. **Ошибка чтения панели в этот момент проглатывается.** В коде прямо
   написано: «a failed read only means it is remembered later, not a
   failed link» — но «later» не наступает: никакой повторной попытки нет.
3. Если человек купил гигабайты в боте ПОСЛЕ связки, сущности в момент
   связки ещё не было, и привязать её тоже некому.
4. Сущность не опознаётся, если в панели у неё не проставлено поле
   `telegramId`: сверка идёт по паре «имя + telegramId».

Вывод: привязка обхода — разовое событие без повторов, и любой сбой в
эту секунду оставляет человека без ключа 2 навсегда.

**ВАЖНО НЕ ПЕРЕПУТАТЬ ДВА ПУТИ.** Подхват обхода ПРИ УСЫНОВЛЕНИИ (вход
на сайт по почте, аккаунта на сайте ещё нет) сделан 20.09.2026 и
работает. Живой случай владельца пришёл ДРУГИМ путём — через СВЯЗКУ
(`linkTelegramAccount`), где обход ищется один раз и без повторов.
Чинить надо именно этот путь; строка в таблице 3а закрыта и к нему не
относится.

### 16.3. Что делает сайт (наша работа)

**Самолечение.** Если у аккаунта есть `telegram_id`, а
`bypass_panel_user_id` пуст — сайт сам ищет `{telegram_id}` в панели и
привязывает. Проверка дешёвая, идемпотентная и выполняется:

- при открытии кабинета (после отрисовки, не задерживая экран);
- проходом воркера для связанных аккаунтов без ключа 2.

**Ошибку чтения больше не проглатывать:** не нашли обход при связке —
пометить аккаунт как «ключ 2 не проверен» и попробовать снова, а не
считать, что его нет.

**Ничего не создавать.** Если сущности `{telegram_id}` в панели нет —
значит у человека нет обхода, и заводить сайтовую `ST…_bp` вместо неё
нельзя: гигабайты живут в ботовской.

### 16.4. Выбор стороны при отвязке

**Как сейчас:** отвязка молча оставляет подписку на сайте. У бота она
исчезает, и его следующая покупка заводит новый ключ. Человека никто не
спрашивает.

**Как надо:** спрашивать. Отвязка — это разделение одного имущества на
две стороны, и решать, где оно останется, должен владелец.

**Два варианта, и оба безопасны:**

| Выбор | Подписка | Ключ в приложении |
|---|---|---|
| «Оставить на сайте» | остаётся на сайте, в боте её нет | **работает, ничего не менять** |
| «Оставить в боте» | возвращается боту, на сайте её нет | **работает, ничего не менять** |

**ГЛАВНОЕ, ЧТО НАДО СКАЗАТЬ ЧЕЛОВЕКУ:** ключ не меняется ни в одном из
вариантов. Сущность в панели одна и та же, меняется только то, **где вы
дальше управляете подпиской и куда платите за продление**. Настроенное
приложение продолжит работать как работало.

Тексты (одинаковые с обеих сторон):

> **Где оставить подписку?**
> Ключ продолжит работать в любом случае — выберите, где вам удобнее
> платить и видеть срок.
> **[На сайте]** — продление и срок в личном кабинете.
> **[В боте]** — продление и срок здесь, как было раньше.

**ГИГАБАЙТЫ ОБХОДА УЧАСТВУЮТ В ВЫБОРЕ НАРАВНЕ С ПОДПИСКОЙ**
(владелец, 22.09.2026 — отменяет прежнее правило этого раздела).

| Выбор | Подписка | Гигабайты обхода |
|---|---|---|
| «Оставить на сайте» | остаётся на сайте | **переходят на сайт целиком** |
| «Оставить в боте» | возвращается боту | остаются у бота целиком |

Выбранная сторона получает ВСЁ, проигравшая — ничего. Долей нет,
делить нечего.

> **Что здесь было раньше и почему отменено.** Стояло: «Гигабайты
> обхода в выборе не участвуют. Они живут в ботовской сущности
> `{telegram_id}` и при отвязке остаются у бота — независимо от
> выбора». Из-за этого человек, купивший гигабайты НА САЙТЕ, терял их
> при отвязке: при связке они переливались в ботовскую сущность, а
> назад не возвращались. Разбор жалобы и новая модель —
> `docs/bot/TZ_BYPASS_MERGE.md`, редакция 3.

Перенос при выборе «на сайте» делает БОТ — он единственный писатель
своей сущности (`/dashboard/site-link/bypass-split`). После переноса
бот обязан обнулить лимит своей сущности обхода, а не просто забыть
указатель: иначе при следующей связке гигабайты посчитаются второй раз.

Обход, заведённый сайтом (`ST…_bp`, `bypass_origin='site'`), при
выборе «в боте» остаётся сайту — как и раньше.

### 16.5. Контракт

`POST /api/bot/unlink` получает необязательное поле:

```json
{ "telegramId": "…", "keep": "site" | "bot" }
```

- `keep` не передан → `"site"` (нынешнее поведение, обратная
  совместимость);
- `"site"` — как сейчас: ключ остаётся за аккаунтом сайта, с сущности
  снимается `telegramId`, ставится маркер отвязки;
- `"bot"` — ключ возвращается боту: `telegramId` на сущности
  СОХРАНЯЕТСЯ, маркер `atlas-site:` снимается, у аккаунта сайта
  `panel_user_id` очищается, срок подписки обнуляется. Дни не
  пропадают — они остаются на сущности, которой теперь распоряжается бот.

**Ответ сайта содержит два поля, без которых бот не может продолжить**
(требование команды бота 20.09.2026, принято):

```json
{ "success": true, "data": {
  "keep": "bot",
  "subscriptionEnd": "2026-12-31T10:00:00.000Z",
  "subscriptionUrl": "https://sub.atlassecure.ru/ebfd48fc-0b1…",
  "siteMarkerRemoved": true
}}
```

- **`subscriptionEnd`** — авторитетный срок на момент отвязки. Отвязка
  это единственная секунда, когда владение сроком переходит от сайта к
  боту; если бот возьмёт дату из своего зеркала, он потеряет дни,
  начисленные сайтом после последнего опроса статуса. Дату обязан
  вернуть тот, кто её отдаёт.
- **`subscriptionUrl`** — ключ, оставшийся у человека. По нему
  принимающая сторона сверяет главный инвариант документа: ключ не
  изменился (раздел 17). Разошёлся — значит ключ в приложении в эту
  секунду умер, и об этом надо кричать в лог, а не ждать обращения.

  Добавлено в список обязательных 20.09.2026 по разбору команды бота.
  Поле было в коде сайта, но не в контракте: они построили на нём
  проверку, и она молча не работала, потому что поля в ответе не было
  вовсе. Формально необязательное поле, на котором держится проверка
  инварианта, — это та же тишина, только отложенная: любая правка на
  стороне сайта убрала бы его снова.

  **Отсутствие поля в ответе — само по себе событие**, а не повод
  пропустить проверку. Принимающая сторона обязана записать это в лог
  отдельной строкой.

- **`siteMarkerRemoved`** — снят ли с сущности маркер `atlas-site:`.
  `false` (панель была недоступна) → бот **не** считает сущность своей,
  оставляет флаг связки и говорит человеку «не получилось, попробуйте
  позже». Честно не довести отвязку лучше, чем начать писать в
  сущность, которую сайт всё ещё считает своей.

Отвязка из кабинета сайта принимает тот же `keep` и ведёт себя так же.

### 16.6. Что делает бот

1. В своём экране отвязки показать вопрос и две кнопки, текстом из 16.4.
2. Передать выбор в `POST /api/bot/unlink` полем `keep`.
3. `keep: "bot"` → сущность снова его: вернуть строку `subscriptions` в
   рабочее состояние по сроку из ответа, дальше вести её самостоятельно.
4. `keep: "site"` → как сейчас.
5. **Ключи обхода не трогать ни при каком выборе.**

### 16.7. Чек-лист

- [ ] Обход, купленный в боте ПОСЛЕ связки, появляется на сайте сам, без
      повторной привязки.
- [ ] Связка при недоступной панели: обход привязывается следующим
      проходом, а не теряется навсегда.
- [ ] Отвязка с `keep: "site"` — как раньше; ключ в приложении работает.
- [ ] Отвязка с `keep: "bot"` — срок и ключ у бота, на сайте подписки
      нет, аккаунт цел; ключ в приложении работает.
- [ ] Гигабайты обхода остаются у бота при обоих выборах.
- [ ] `keep` не передан → поведение прежнее.
- [ ] `keep: "bot"` при недоступной панели → `siteMarkerRemoved: false`,
      бот не берёт сущность, отвязка не считается состоявшейся.
- [ ] `subscriptionEnd` в ответе совпадает с тем, что было на сайте в
      момент отвязки, а не со сроком из зеркала бота.
- [ ] `subscriptionUrl` присутствует в ответе при обоих значениях `keep`
      и совпадает с ключом, который был привязан.
- [ ] Ответ БЕЗ `subscriptionUrl` (сломали контракт) — принимающая
      сторона пишет об этом в лог, а не пропускает проверку молча.

### 16.8. Порядок работ

Разделяем, как предложила команда бота:

1. **Сначала обход** (16.3) — мешает живым людям прямо сейчас, сторона
   сайта, бота не касается.
2. **Потом выбор при отвязке** (16.4–16.6) — ничего не блокирует, идёт
   после согласования контракта.

---

## 17. Ключ не меняется. Никогда

Решение владельца 20.09.2026, отменяет часть принципа 4 и раздела 16.

> «Если у пользователя был реальный нормальный ключ в боте и он
> привязывает почту, а потом её отвязывает — или вообще любое действие —
> ключ не должен меняться. И наоборот: был на сайте, привязал Telegram,
> отвязал Telegram — ключ не должен меняться. Должна меняться только
> почта. И всё.»

Это правило старше всех остальных в документе. Где оно спорит с чем-то —
право оно.

### 17.1. Почему это важнее удобства разработки

Ключ прописан у человека в приложении на телефоне, ноутбуке и телевизоре.
Он его не помнит, не хранит и часто не понимает, что это такое. Смена
ключа для него — «интернет сломался», и виноват в этом не он, а мы: он
всего лишь нажал кнопку, которую мы ему показали.

Поэтому любое наше внутреннее действие — связка, слияние, отвязка, выбор
стороны — **не имеет права трогать работающий ключ**.

### 17.2. Где правило нарушается сегодня

**При слиянии.** `decideMerge` оставляет сущность с БОЛЬШИМ сроком, а
вторую гасит (`DISABLED`). Если человек пришёл из бота с рабочим ключом,
а на сайте у него оказалась подписка подлиннее — гасится именно ботовский
ключ, тот самый, что стоит у него в приложении. Человек нажал «привязать
почту» и остался без интернета.

**При отвязке.** Ключ остаётся за сайтом всегда, даже если человек всю
жизнь пользовался ботом и туда же вернулся.

### 17.3. Новое правило: выживает ключ той стороны, откуда пришёл человек

Сторона известна из `via`, её не нужно угадывать:

| `via` | Откуда человек | Какой ключ остаётся |
|---|---|---|
| `bot_email` | ввёл почту в боте | **ботовский** `tg_{id}_premium` |
| `site_token` | нажал «Привязать Telegram» в кабинете | **сайтовый** `ST…` |
| `relink` | сверка, человека рядом нет | прежнее правило: больший срок |
| `adopted` | вход на сайт по почте, аккаунта не было | ботовский (других нет) |

Человек стоит на одной из двух сторон, и в приложении у него ключ именно
этой стороны. Он её и оставляет.

**Срок при этом не теряется:** дни складываются (принцип 4), так что
выбирать «подлиннее» больше не нужно — эта причина исчезла вместе с
прежним правилом.

Проигравшая сущность гасится `DISABLED` как и раньше — но теперь это
всегда тот ключ, которым человек в эту секунду НЕ пользуется.

### 17.4. При отвязке — выбор, и снова без смены ключа

Владелец: «пользователь в боте изначально был, привязывает почту, потом
решает отвязать — он привык, что у него всё в боте, а мы его без
предупреждения кидаем на сайт».

Спрашиваем, где оставить подписку: **на сайте** или **в боте**. Контракт
— раздел 16.5 (`keep`, `subscriptionEnd`, `siteMarkerRemoved`).

**И в том, и в другом случае ключ остаётся тем же.** Сущность в панели
одна, меняется только владелец записи: кто ведёт срок и куда человек
платит. Это надо сказать ему прямо, иначе выбор пугает:

> **Где оставить подписку?**
> Ключ продолжит работать в любом случае — ничего перенастраивать не
> нужно. Выберите, где вам удобнее платить и видеть срок.
> **[На сайте]** · **[В боте]**

**Умолчание — сторона, где человек находится.** Отвязывает в боте —
предложен «В боте». В кабинете — «На сайте». Не наоборот.

### 17.5. Никаких новых ключей

Отдельно, потому что владелец спросил прямо: **связка НЕ создаёт новую
сущность в панели.** Ни при привязке почты, ни при привязке Telegram.

Происходит ровно одно: к существующей сущности дописывается недостающее
поле — почта к ботовской, `telegramId` к сайтовой. Сущность, ключ,
`subscriptionUrl` остаются те же.

Новый ключ появляется только там, где иначе нельзя: человек отвязался,
живого ключа у стороны не осталось, и он покупает заново. Это не смена
ключа, а покупка после её отсутствия.

### 17.6. Что меняется в коде

**Сайт:**
- `decideMerge` получает сторону (`via`) и оставляет её ключ; правило
  «больший срок» остаётся только для `relink`;
- отвязка принимает `keep` (16.5) и умеет отдать сущность боту;
- тексты выбора из 17.4 в кабинете.

**Бот:**
- экран выбора при отвязке с умолчанием «В боте»;
- передача `keep`, обработка ответа по 16.5;
- ничего не создавать и не гасить самому — обе стороны меняет сайт.

### 17.7. Чек-лист

- [ ] Бот-первым со связкой: ключ в приложении не перестал работать.
- [ ] Сайт-первым со связкой: то же самое.
- [ ] Обе стороны с живой подпиской, пришёл из бота → остался ботовский
      ключ, даже если на сайте срок был длиннее.
- [ ] Обе стороны с живой подпиской, пришёл с сайта → остался сайтовый.
- [ ] Дни в обоих случаях — сумма остатков.
- [ ] Отвязка с любым `keep`: `subscriptionUrl` до и после совпадает.
- [ ] За всю связку и отвязку в панели не создано ни одной новой сущности.
