...
**Гигабайты обхода в выборе не участвуют.** Они живут в ботовской
сущности `{telegram_id}` и при отвязке остаются у бота — независимо от
выбора. Исключение: обход, заведённый сайтом (`ST…_bp`,
`bypass_origin='site'`), остаётся у сайта. Так работает и сейчас.
