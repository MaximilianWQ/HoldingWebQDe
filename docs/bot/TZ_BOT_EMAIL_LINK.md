...
**Настройка перед включением (делает владелец, агент перечисляет в отчёте):**
1. Секрет: `BOT_API_KEY` в Railway сайта = `PROD_SITE_BOT_API_KEY` в Railway
   бота (бот читает переменные с префиксом окружения, `config.env`).
