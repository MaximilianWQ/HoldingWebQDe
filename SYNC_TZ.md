...
> **Для бота главный документ — `docs/bot/TZ_BOT_EMAIL_LINK.md` (17.09.2026).**
> Этот файл — справочник по балансу, рефералам и подписке. Отличия, которые
> действуют для бота: `extend` — без `amount` (кешбэк за оплаты в боте платит
> бот), `paymentId` обязателен; `sync-balance` — только двухфазный режим
> (`twoPhase` + `ack`, раздел 1a); `sync` — только отзыв (`plan: "none"`) и
> смена тарифа, дата раньше текущей → `409 WOULD_SHORTEN`; `sync-referrals` —
> необязателен. Шкала кешбэка ниже (10/25/45) — сайта; у бота своя, решение
> владельца — раздел 13 главного документа.
...
### 1a. Двухфазный режим (17.09.2026) — единственный для бота

Прежний режим выше помечает кешбэк отданным в момент ответа: потерянный в
сети ответ терял деньги. Двухфазный:

1. `POST /api/bot/sync-balance { telegramId, balance, twoPhase: true }` —
   тот же ответ, но записи НЕ помечаются; повтор вернёт их снова.
2. Бот зачисляет каждую запись с дедупликацией по `id` (своя таблица
   применённых id).
3. `POST /api/bot/sync-balance { telegramId, ack: ["tx-id", …] }` →
   `{ acked: [...], ackedCount }` — помечает отданными ровно эти записи.
...
**Логика:** берёт MAX от бота и сайта по каждому полю — данные никогда не теряются. Для бота вызов необязателен.
...
**С 17.09.2026:** дата раньше текущего срока сайта не применяется —
`409 {code: "WOULD_SHORTEN", data: {subscriptionEnd: <текущий>}}`. Отзыв —
`plan: "none"`. Продлевать этим вызовом нельзя — только `extend`.
