...
### Изменение 20.09.2026 — постоянный токен привязки отменён

По итогам аудита безопасности сайт БОЛЬШЕ НЕ ПРИНИМАЕТ постоянный
токен привязки (16 шестнадцатеричных знаков, поле
`users.telegram_link_token`). Причина: он выдавался при создании
аккаунта, жил вечно, хранился открытым текстом и уезжал в браузер при
каждой загрузке кабинета — то есть был вечным ключом от аккаунта,
который легко утекал.

Что это значит для бота:

- разбирать в `/start` только `^link_[A-Za-z0-9_-]{43}$` — одноразовый
  токен на 15 минут. Ветки под 16-значный токен не делать;
- `kind` в ответе `GET /api/bot/link` теперь всегда `"one_time"`;
- поля `telegramLinkToken` в ответах сайта больше нет.

### Вне этого ТЗ

- *(снято 20.09.2026 — теперь это В ТЗ, раздел 4а: кнопка «Войти через
  Telegram» на сайте сделана и работает.)*
...
## 0. Инструкция агенту бота

Ты реализуешь сторону **бота**. Сайт готов и в этой работе не меняется.
Если контракт сайта где-то не сходится с кодом бота или чего-то не хватает —
не додумывай и не обходи: опиши расхождение в отчёте владельцу.

**Прочитать до начала:** этот документ целиком; `CLAUDE.md` бота и вложенные
`app/services/payments/CLAUDE.md`, `database/CLAUDE.md`, `app/handlers/CLAUDE.md`;
`docs/WORKFLOWS.md`. Номера строк в ТЗ — на `4b00e79a`; если `main` ушёл
вперёд, ищи по имени функции.

**Правила репозитория, которые здесь особенно важны:**
- никакого HTTP при открытом соединении или транзакции БД — вызовы сайта
  только до или после транзакции (задача очереди — вне транзакции);
- ретраи — только `retry_async`, один слой: внутри задачи очереди HTTP не
  ретраится, ретраит сама очередь;
- финансовые инварианты: ни двойной активации, ни двойного начисления, ни
  потери подписки; мутация денег — одна транзакция, один коннекшн;
- логика — в `app/services/site_link/service.py`, HTTP — в
  `app/services/site_link/client.py`, хендлеры и воркер — тонкие;
- тексты — только `get_text`, ключи в `ru.py` и `en.py`; отправка —
  `safe_send_message`; логи — `log_event` с тегом `SITE_LINK_*`, без почты
  целиком, без кодов из писем, без ключа API;
- время — UTC через `_to_db_utc()` / `_from_db_utc()`;
- миграция применяется на пустой БД и обратно совместима;
- новые админ-действия — только в дашборде (`app/api/dashboard/`).

**Этапы — отдельными PR, по порядку:**

| Этап | Что | Включает ли поведение |
|---|---|---|
| A. Защита | миграция 096 (12.2), `site_link.is_linked`, развилка 5.2, все защиты раздела 6, `atlas-site:` в `_is_our_entity` (6.3), `premium_minutes` в задачах минутных выдач | нет: `site_linked` ни у кого не стоит, поведение несвязанных не меняется — это и надо доказать тестами |
| B. Клиент и связка | `client.py`, `service.py`, сценарии раздела 4, `/start link_`, кнопка в профиле, тексты | за флагом `SITE_LINK_ENABLED` (по умолчанию выключен) |
| C. Зеркало, кешбэк, ключи | воркер раздела 7 и 9, выдача ключей раздела 10, обход раздела 8, экраны после оплаты 5.3 | за тем же флагом |
| D. Сверка | проход 12.4 — действие в дашборде «Сверить связки с сайтом» с отчётом | запускает админ |

**Проверка:**
- юнит-тесты по образцу `tests/services/*`: клиент сайта подменяется
  `monkeypatch` на уровне функций `client.py`; на каждый `code` из раздела 3 —
  своя ветка; развилка `apply()` — связанный уходит в `extend`, несвязанный
  идёт прежним путём без единого изменения;
- ручная проверка после выкладки B–C — на отдельном тестовом Telegram и
  тестовой почте против https://qodev.dev (другого стенда у сайта нет), по
  чек-листу раздела 15;
- боевых пользователей до этапа D не трогать.

### 0.1. Первый ответ агента — переменные окружения (до кода)

Прежде чем писать код, пришли владельцу таблицу «переменная → в каком
сервисе Railway → значение или откуда взять → обязательна ли». Возьми за
основу таблицу ниже, сверь с `config.py` бота (все переменные бота читаются
через `config.env()` с префиксом окружения: `PROD_…` на проде, `STAGE_…` на
стейдже) и допиши, если по ходу чтения кода нашлось что-то ещё.

**Railway бота** (`APP_ENV=prod`):

| Переменная | Значение | Обязательна | Когда |
|---|---|---|---|
| `PROD_SITE_BOT_API_KEY` | секрет: то же значение, что `BOT_API_KEY` у сайта; сгенерировать одну длинную случайную строку (≥ 32 символа) и положить в оба сервиса | да | до этапа B |
| `PROD_SITE_API_BASE_URL` | `https://qodev.dev` — умолчание в коде, задавать не нужно | нет | — |
| `PROD_SITE_LINK_ENABLED` | `false` при выкладке B–C, `true` после проверки на тестовом аккаунте | да | после проверки |
| `PROD_REMNAWAVE_CLIENTS_SQUAD_UUID` (или `PROD_REMNAWAVE_SQUAD_UUID`) | `6947418d-83b6-4050-a10c-3829e4cd4b2c` — сквад обхода, должен совпадать с сайтом; проверить текущее значение | да | этап A |
| `PROD_REMNAWAVE_BYPASS_DEVICE_LIMIT` | `5` (умолчание в коде); если задано — должно быть 5 | нет | проверить |
| `PROD_REMNAWAVE_PREMIUM_USERNAME_PATTERN` | не задавать (умолчание `tg_{telegram_id}_premium` совпадает с сайтом); если задано иначе — сообщить | нет | проверить |
| `PROD_REMNAWAVE_BYPASS_USERNAME_PATTERN` | не задавать (умолчание `{telegram_id}` совпадает с сайтом); если задано иначе — сообщить | нет | проверить |
| `PROD_BOT_USERNAME` | `atlassecure_bot` (умолчание); сайт открывает этого бота по ссылке привязки | нет | проверить |
| `PROD_REMNAWAVE_API_URL`, `PROD_REMNAWAVE_API_TOKEN` | уже стоят (та же панель `https://rmnw.atlassecure.ru`, что у сайта) | — | не менять |

**Railway сайта:**

| Переменная | Значение | Обязательна |
|---|---|---|
| `BOT_API_KEY` | тот же секрет, что `PROD_SITE_BOT_API_KEY` у бота | да |
| `REMNAWAVE_BYPASS_SQUAD_UUID`, `REMNAWAVE_BYPASS_DEVICE_LIMIT`, `BOT_PREMIUM_USERNAME_PATTERN`, `BOT_BYPASS_USERNAME_PATTERN`, `TELEGRAM_BOT_USERNAME` | не задавать: умолчания в коде сайта (`6947418d-…`, `5`, `tg_{telegram_id}_premium`, `{telegram_id}`, `atlassecure_bot`) совпадают с ботом | нет |

**Админка сайта:** `/admin` → «Сервис» → «Синхронизация с ботом» — включена (по
умолчанию включена; выключенная отвечает боту `503 bot_sync_disabled`).

Секрет не присылать в чат и не коммитить — только в Railway.

### 0.2. Настройка перед включением (делает владелец)

1. Один секрет в оба сервиса: `BOT_API_KEY` (сайт) = `PROD_SITE_BOT_API_KEY` (бот).
2. `PROD_REMNAWAVE_CLIENTS_SQUAD_UUID` бота = `6947418d-83b6-4050-a10c-3829e4cd4b2c`.
3. `PROD_SITE_LINK_ENABLED=true` — только после проверки этапов B–C.
4. Решения раздела 13.

**Отчёт по каждому этапу:** что сделано (файлы), какие пункты разделов 5–10
закрыты, результаты тестов, расхождения с ТЗ и вопросы владельцу.
...
4. **Слияние при связке делает сайт. Дни СКЛАДЫВАЮТСЯ** (решение
   владельца 20.09.2026, заменяет прежнее «остаётся больший срок»). Есть
   подписка и в боте, и на сайте — остаётся ОДНА сущность, а её срок
   равен сумме остатков обеих. Вторая `DISABLED`.

   Причина смены правила: человек заплатил дважды, и прежнее правило
   молча отнимало у него оплаченное. Складывать — единственное, что не
   требует объяснений в поддержке.

   Что складывается — ОСТАТКИ, а не полные сроки: если у сайта 40 дней
   впереди, а у бота 120, получается 160 от сегодняшнего дня. Истёкшая
   подписка даёт ноль, а не отрицательные дни.

   Считает сайт, одним событием журнала с `setEnd` (абсолютная дата).
   Предел в 400 дней на одно событие (`MAX_SINGLE_EXTENSION_MS`) к
   `setEnd` не применяется, так что сумма больше года проходит.
...
6. **Обход (bypass) — продукт бота**, сущность `{telegram_id}` ведёт бот;
   ...
   **Связанный человек видит на сайте ОБА ключа бота**: премиум
   `tg_{id}_premium` и обход `{id}`. Своих `ST…` / `ST…_bp` у него не
   появляется, перенастраивать в приложении нечего.
...
`200 {valid: true, kind: "one_time", maskedEmail, expiresAt, telegramLinked}`;
...
## 3а. Что сайт уже умеет: вход по почте находит ключ в панели

Проверено по коду сайта 20.09.2026 — реализовано и работает
(`adoptVerifiedPanelAccount`, `src/lib/telegram-link.ts`).

Человек входит на сайт по коду из письма. Аккаунта на сайте нет. Сайт:

1. ищет в панели по адресу почты — `GET /api/users/stream?email=…`
   (`findUsersByEmail`, `src/lib/remnawave.ts`);
2. берёт только сущности с маркером `atlas-email-verified` в описании,
   не bypass и не `DISABLED`; из нескольких — с самым дальним сроком;
3. заводит аккаунт сайта поверх НАЙДЕННОЙ сущности: её `subscriptionUrl`
   становится ключом человека, её срок — сроком подписки, пробный период
   не выдаётся, Telegram ID подхватывается, если он свободен.

**Отсюда работа бота — одна:** после подтверждения почты кодом записать
эту почту в свои сущности в панели и поставить маркер
`atlas-email-verified`. Дальше человеку достаточно войти на сайт той же
почтой — сайт сам отдаст его ключ.

**ПОЧЕМУ БЕЗ КОДА НЕЛЬЗЯ.** Почта в панели — это ключ к выдаче ключей.
Если бот запишет адрес просто потому, что человек его ввёл, то любой
введёт чужой адрес, а владелец этого адреса, войдя на сайт, получит
чужие ключи — или наоборот, лишится своих. Маркер `atlas-email-verified`
и существует затем, чтобы сайт отличал доказанный адрес от введённого;
сущность без маркера сайт не усыновляет.

**Чего на сайте пока НЕТ и что нужно дописать (наша сторона):**

| Что | Сейчас | Нужно |
|---|---|---|
| Ключ обхода при усыновлении | берётся только премиум-сущность, обход остаётся ботовским и на сайте не показывается | подхватывать и `{telegram_id}`: он становится ключом 2 на сайте |
| Сложение дней | при слиянии остаётся больший срок | складывать остатки (принцип 4) |

## 3б. Сопоставление по снимкам панели (владелец, 20.09.2026)

Владелец прислал обе сущности одного человека из панели. Сверено:

| Поле в панели | Обход | Премиум |
|---|---|---|
| `username` | `1273979592` — голый Telegram ID | `tg_1273979592_premium` |
| id панели | 64934 | 64933 |
| Telegram ID | `1273979592` | `1273979592` |
| **Email** | **пусто** («Enter user's email (optional)») | **пусто** |
| Лимит трафика | 500 MiB, «Не сбрасывать» | 0 = без ограничений |
| Дата окончания | 01.01.2100 (обход временем не ограничен) | 23.09.2026 |
| Внутренний сквад | `Clients` | `MainServer` |

**Поле Email в панели есть у обеих сущностей и сейчас пустое — его и
заполняет бот** после подтверждения почты кодом. Заполнять нужно
ОБЕ: сайт ищет по адресу и должен найти и ключ, и обход.

Вместе с адресом в описание ставится маркер `atlas-email-verified` —
по нему сайт отличает доказанный адрес от введённого.

**ВАЖНО ПРО РАЗЛИЧЕНИЕ СУЩНОСТЕЙ.** Сайт узнаёт обход по описанию
`Bypass via bot (…)`, а теперь ещё и по имени из одних цифр,
совпадающему с полем Telegram ID (правка сайта 20.09.2026). Второй
признак добавлен потому, что описание можно не проставить или стереть
правкой в панели руками — и тогда сайт принял бы обход с лимитом
500 МБ за премиум-ключ и отдал его человеку как подписку. Бот должен
сохранять оба признака: имя `{telegram_id}` и описание
`Bypass via bot (…)`.
...
| `kept` | Сообщение |
|---|---|
| `bot` | «Готово! Бот и сайт теперь работают с одной подпиской. Ваш ключ из бота остаётся — ничего перенастраивать не нужно. Дни с сайта добавлены: подписка до {дата}.» |
...
## 4а. Обратная сторона: сайт → бот (владелец, 20.09.2026)

Обе кнопки на сайте сделаны и работают. Оба потока начинает САЙТ,
поэтому бот ничего не расшифровывает — он спрашивает сайт, кто это.

### 4а.1. «Привязать Telegram» из кабинета

1. Человек в кабинете жмёт «Привязать Telegram».
2. Сайт создаёт ОДНОРАЗОВЫЙ токен на 15 минут
   (`POST /api/user/telegram-link`) и открывает
   `https://t.me/atlassecure_bot?start=link_<токен>`.
3. Бот в `cmd_start` разбирает `^link_[A-Za-z0-9_-]{43}$` и спрашивает
   сайт: `GET /api/bot/link?token=<токен>`.
4. **Сайт отвечает, чей это токен:** `maskedEmail`, `expiresAt`,
   `telegramLinked`. Больше бот ничего знать не должен — ни адреса
   целиком, ни идентификатора аккаунта.
5. Бот показывает: «Привязать аккаунт {maskedEmail} к этому Telegram?»
   [✅ Привязать] [Отмена].
6. «Привязать» → `POST /api/bot/link {token, telegramId,
   premiumPanelUserId}`. Отмена — токен не тратится.

**Ответ на вопрос «как бот поймёт, кто это»:** никак сам — по токену
спрашивает сайт. Токен одноразовый и живёт 15 минут, поэтому им нельзя
воспользоваться дважды или позже.

### 4а.2. «Войти через Telegram» на странице входа

Сделано на сайте 20.09.2026; серверная часть была готова раньше.

1. Человек на `/auth` жмёт «Войти через Telegram».
2. Сайт создаёт ключ входа, привязанный К ЭТОМУ БРАУЗЕРУ (секрет в
   httpOnly cookie, в базе только его хеш), и открывает
   `https://t.me/atlassecure_bot?start=tglogin_<nonce>`.
   На экране сайта появляются ЧЕТЫРЕ ЦИФРЫ.
3. Бот разбирает `^tglogin_[A-Za-z0-9_-]{16,64}$` и вызывает
   `POST /api/bot/auth-login {nonce, telegramId}`.
   **Сайт возвращает те же четыре цифры**, а также IP и браузер, с
   которых просят вход.
4. Бот показывает их человеку: «Подтвердить вход на сайте? Код 4821,
   устройство …». [✅ Подтвердить] [Отмена].
5. Подтверждение → бот сообщает сайту, что ключ подтверждён; страница
   сайта это видит и впускает человека.

**ЗАЧЕМ ЧЕТЫРЕ ЦИФРЫ.** Без них злоумышленник открывал бы вход у себя
и присылал жертве ссылку `t.me/…?start=tglogin_<свой nonce>`: одно
нажатие — и он в чужом аккаунте. Цифры на экране сайта и в боте должны
совпасть, а совпасть они могут только у того, кто сам начал вход.
Бот обязан показывать код и спрашивать подтверждение — молча
подтверждать нельзя.

**ЕСЛИ АККАУНТА НА САЙТЕ ЕЩЁ НЕТ.** Сайт заводит его сам и переносит
всё, что есть у этого Telegram ID: подписку, ключ и обход. Человеку
ничего вводить не нужно. Почта в таком аккаунте появится, когда он её
привяжет.
...
- `app/services/site_link/service.py` — `is_linked(tg)` (локальный флаг),
  `refresh_status(tg, max_age=60)`, `apply_link_result(tg, data)` (4.5),
  `apply_unlink(tg)` (4.6), запись зеркала (7), обмен кешбэком (9).
...
- `tests/services/test_site_link_client.py`, `tests/services/test_site_link_service.py`,
  `tests/handlers/test_site_link.py`, тест развилки `apply()`
  (имена — по месту, главное покрытие из раздела 15).
...
Полная таблица — раздел 0.1.
- `SITE_API_BASE_URL` — по умолчанию `https://qodev.dev` (в коде).
- `SITE_BOT_API_KEY` — секрет, равен `BOT_API_KEY` сайта (в Railway бота — `PROD_SITE_BOT_API_KEY`).
...
2. **Бот, этап A — защита** (раздел 0): без изменения поведения.
3. **Бот, этапы B–C** за выключенным флагом; настройка секрета; проверка на
   тестовом аккаунте; затем `PROD_SITE_LINK_ENABLED=true`.
4. **Этап D — сверка 12.4** для уже связанных.
