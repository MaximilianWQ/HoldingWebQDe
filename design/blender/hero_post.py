...
# ── Растворить нижний край ────────────────────────────────────────
# Тень на полу уходит к камере и доходит до нижнего края кадра: даже
# после чистки там остаётся alpha ~20, а это прямая серая линия поперёк
# картинки («резкий переход с рендеров», владелец 18.09.2026). Поднять
# камеру нельзя — пол всегда доходит до низа кадра, поэтому последние
# проценты высоты гасим ступенчатой лесенкой: тень просто растворяется.
FEATHER = 0.09
rows = max(1, round(im.height * FEATHER))
alpha = im.getchannel("A")
for i in range(rows):
    y = im.height - rows + i
    k = 1.0 - (i + 1) / rows
    line = alpha.crop((0, y, im.width, y + 1)).point(lambda v, k=k: int(v * k))
    alpha.paste(line, (0, y))
im.putalpha(alpha)
